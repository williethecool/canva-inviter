...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/williethecool/canva-inviter)

*Cloudflare will clone the repo to your GitHub account, provision the KV namespace, prompt you for your secrets, and wire up automatic CI/CD — all in one flow. No terminal required.*

---

[**Report a Bug**](https://github.com/williethecool/canva-inviter/issues/new?template=bug_report.md) · [**Request a Feature**](https://github.com/williethecool/canva-inviter/issues/new?template=feature_request.md) · [**Contributing Guide**](CONTRIBUTING.md)
...
├── wrangler.toml           # Wrangler config — KV binding + deploy button support
├── package.json            # Deploy button binding descriptions + npm scripts
...
└── docs/
    ├── deployment-guide.md # Step-by-step GUI deployment (no CLI)
    └── security.md         # Threat model and security decisions
```

> **One-click or no-click.** Use the [Deploy to Cloudflare](#-one-click-deploy) button for a fully automated setup, or paste `worker.js` directly into the Cloudflare dashboard editor for a manual deploy. Either way, there is no build step and no local tooling required.

---

## Deployment

There are two ways to deploy: the **one-click button** (recommended) or the **manual GUI walkthrough** below if you prefer full control.
...
Clicking the button above launches Cloudflare's guided deploy flow, which will:

1. **Clone** this repository into your own GitHub account so you own the code
2. **Provision** the `INVITE_KV` namespace automatically — no manual KV setup
3. **Prompt** you for the four required secrets in a single form:
   ...
   | `SESSION_SECRET` | A 64-char random hex string *(see generator below)* |

4. **Deploy** the Worker to your Cloudflare account
5. **Wire up CI/CD** — every future push to `main` redeploys automatically

**Generating `SESSION_SECRET`** — paste this into any browser console:
```javascript
Array.from(crypto.getRandomValues(new Uint8Array(32)))
  .map(b => b.toString(16).padStart(2, '0')).join('')
```

> After deployment, visit `https://canva-invite-manager.YOUR-SUBDOMAIN.workers.dev/admin/login` and sign in with the credentials you entered above.

---

### Option B — Manual GUI Walkthrough
...
- Search [existing issues](https://github.com/williethecool/canva-inviter/issues) to avoid duplicates
